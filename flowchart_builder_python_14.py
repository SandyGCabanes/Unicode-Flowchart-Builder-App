...
class FlowchartBuilderApp:
    ...
    MAX_TEXT_LINES = UNIFORM_NODE_HEIGHT - 2 # Interior lines available inside a box (total height minus top/bottom border)

    def __init__(self, master):
        self.master = master
        master.title("Unicode Flowchart Builder - Sandy G. Cabanes")
        ...
        self.lbl_char_limit_note = ttk.Label(self.input_frame, text=f"(Max {self.UNIFORM_NODE_WIDTH - 4} chars per line, up to {self.MAX_TEXT_LINES} lines - {self.MAX_TEXT_LINES - 1} if marked as a loop - text will auto-wrap)", font=("TkDefaultFont", 8, "italic"))
        ...
        self.rb_left = ttk.Radiobutton(self.input_frame, text="Left", variable=self.branch_direction_var, value="left")
    ...
    def _update_input_layout(self):
        ...
        if not self.nodes:
            ...
            self.rb_left.grid(row=current_row, column=0, sticky="w", padx=(0, 10)); current_row += 1
    ...
    def add_start_node(self):
        ...
        # FIXED: The start node never has a loop note, so it can use every
        # line the box has room for, instead of being capped at 2 regardless.
        node_text_lines = self._wrap_text(raw_text, self.UNIFORM_NODE_WIDTH - 4)[:self.MAX_TEXT_LINES]
    ...
    def add_next_node(self):
        ...
        # FIXED: Only reserve a line for the "*Loop to:" note when this step is
        # actually a loop. Non-loop steps now get the full box capacity instead
        # of always being capped as if a loop note was coming.
        is_loop = self.is_loop_var.get()
        max_text_lines = self.MAX_TEXT_LINES - 1 if is_loop else self.MAX_TEXT_LINES
        node_text_lines = self._wrap_text(raw_text, self.UNIFORM_NODE_WIDTH - 4)[:max_text_lines]
    ...
    def _compute_child_position(self, source_pos, direction, node_width, node_height):
        """
        Single source of truth for 'where does the next box go' math.
        Down, Right, and Left all read from here so adding a future
        direction only means adding one more branch in this one method.
        """
        if direction == 'right':
            return source_pos['x'] + source_pos['width'] + self.HORIZONTAL_SPACING, source_pos['y']
        elif direction == 'left':
            return source_pos['x'] - node_width - self.HORIZONTAL_SPACING, source_pos['y']
        else:  # 'down' and any unrecognized value default to Down for safety
            return source_pos['x'], source_pos['y'] + source_pos['height'] + self.VERTICAL_SPACING
    ...
    def generate_flowchart(self):
        ...
        node_positions = {}; max_x, max_y, min_x = 0, 0, 0
        ...
        for node in self.nodes:
            ...
            if not node['connections']: x, y = 0, 0
                ...
                if node['type'] == 'merge' or (node['is_end'] and len(sources_pos) > 1):
                    ...
                    x, y = self._compute_child_position(source_pos, node['direction'], node_width, node_height)
            ...
            max_x = max(max_x, x + node_width); max_y = max(max_y, y + node_height); min_x = min(min_x, x)

        # NEW: "Left" branches can produce negative x positions. Shift every node
        # rightward by the same amount so the whole drawing lands back on the
        # visible page, the same way a page in a word processor auto-expands
        # instead of letting content run off the left edge.
        if min_x < 0:
            x_shift = -min_x
        ...
        for pos_data in node_positions.values():
            ...
            if node['type'] == 'merge' or (node['is_end'] and len(sources_pos) > 1):
                ...
                if node['direction'] == 'down':
                    ...
                    for i in range(x + w, s_pos['x']): draw_char(i, sy_center, '─'); draw_char(x + w, sy_center, '◄')
                else:
